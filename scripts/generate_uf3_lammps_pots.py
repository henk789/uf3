...
if len(sys.argv) != 3:
    ...
    raise ValueError(
        "Invalid number of arguments. Enter UF3 model file and name\n\
...
def write_uf3_lammps_pot_files(chemical_sys, model, pot_dir):
    ...
    for interaction in chemical_sys.interactions_map[2]:
        ...
        files[key] += (
            str(model.bspline_config.r_max_map[interaction])
            + " "
        ...
        files[key] += (
            " ".join(
                [
                    "{:.17g}".format(v)
                    for v in model.bspline_config.knots_map[interaction]
                ]
            )
            + "\n"
        )
        ...
        files[key] += (
            str(model.bspline_config.get_interaction_partitions()[0][interaction])
            + "\n"
        )
        ...
        files[key] += (
            " ".join(
                [
                    "{:.17g}".format(v)
                    for v in model.coefficients[start_index : start_index + length]
                ]
            )
            + "\n"
        )
    ...
    if 3 in model.bspline_config.interactions_map:
        for interaction in model.bspline_config.interactions_map[3]:
            ...
            files[key] += (
                str(model.bspline_config.r_max_map[interaction][2])
                + " "
            ...
            files[key] += (
                str(len(model.bspline_config.knots_map[interaction][2]))
                + " "
            ...
            files[key] += (
                " ".join(
                    [
                        "{:.17g}".format(v)
                        for v in model.bspline_config.knots_map[interaction][2]
                    ]
                )
                + "\n"
            )
            ...
            files[key] += (
                " ".join(
                    [
                        "{:.17g}".format(v)
                        for v in model.bspline_config.knots_map[interaction][1]
                    ]
                )
                + "\n"
            )
            ...
            files[key] += (
                " ".join(
                    [
                        "{:.17g}".format(v)
                        for v in model.bspline_config.knots_map[interaction][0]
                    ]
                )
                + "\n"
            )

            solutions = least_squares.arrange_coefficients(
                model.coefficients, model.bspline_config
            )
...
pot_files = write_uf3_lammps_pot_files(
    chemical_sys=chemical_sys, model=model, pot_dir=pot_dir
)
pot_files = list(pot_files)
lines = "pair_style uf3 %i %i" % (
    model.bspline_config.degree,
    len(chemical_sys.element_list),
)
