...
def test_energy():
    ...
    pair = uf2_pair(displacement, coefficients=coefficients, knots=knots, cutoff=knots[-1])
    nf, ef = uf2_neighbor(displacement, box_size, coefficients=coefficients, knots=knots, cutoff=knots[-1])
    ...
    knot_dict[(0,1)] = knots
    knot_dict[(1,1)] = knots
    nfs, efs = uf2_neighbor(displacement, box_size, species=species, coefficients=coeff_dict, knots=knot_dict, cutoff=knots[-1])
...
def test_2_body_interaction_count():
    ...
    knot_dict[(0,1)] = knots
    knot_dict[(1,1)] = knots
    nfs, efs = uf2_neighbor(displacement, box_size, species=species, coefficients=coeff_dict, knots=knot_dict, cutoff=box_size)
...
def test_3_body_interaction_count():
    ...
    knots = jnp.arange(0, box_size*2, 0.25) # for triangles one side might need to span the entire cube diagonally
    knots = jnp.pad(knots, (3,3), 'edge')
